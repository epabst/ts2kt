...
data class TypeUnion(val possibleTypes: List<Type>) : Node() {
    ...
    fun map(function: (Type) -> Type): TypeUnion = TypeUnion(possibleTypes.map(function))

    fun mapLast(function: (Type) -> Type): TypeUnion =TypeUnion(possibleTypes.dropLast(1) + function(possibleTypes.last()))

    fun flatMap(function: (Type) -> TypeUnion): TypeUnion = TypeUnion(possibleTypes.flatMap { function(it).possibleTypes })
}
